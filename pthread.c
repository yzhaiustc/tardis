...
void *threadFun(void *arg) //arg  传递线程标号（自己定义）
{
    ...
    double t1 = get_sec(CLOCK_THREAD_CPUTIME_ID);
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, k, 1.0, A, k, B, n, 1.0, C, n);
    double t2 = get_sec(CLOCK_THREAD_CPUTIME_ID);
    double time_elapsed = t2-t1;
    printf("time elapsed: %f sec, perf: %f GFLOPS, cpu_id: %d\n", time_elapsed, 
            2 * (m / 1000) * (n / 1000) * (k / 1000) / time_elapsed, i);
